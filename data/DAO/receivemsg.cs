...
namespace rfidWebservice.data.DAO
{
    public class receivemsg
    {
        ...
        public MessagePack save()
        {
            ...
            {
                //添加
                sqlstr = "insert into t_receivemsg(serviceid, sourceappcode, msg, sendtime) values(@serviceid,@sourceappcode,@msg,@sendtime)";
            }
            else
            {
                //更新
                sqlstr = "update t_sendmsg set serviceid = @serviceid, sourceappcode = @sourceappcode, msg = @ms, sendtime = @sendtime where id = '" + id + "'";
            }
